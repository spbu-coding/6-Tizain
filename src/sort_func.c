...
void merge(strings_array_t strings_array, array_size_t array_size, comparator_func_t comparator_func) {
    ...
    size_t step = 1;  // шаг разбиения последовательности
    ...
    while (step < array_size)  { // пока шаг меньше длины массива
        ...
        size_t l = 0;      // левая граница участка
        size_t m = l + step;  // середина участка
        size_t r = l + step * 2;  // правая граница участка
        do	{
            ...
            for (; i1 < m && i2 < r; ) { // пока i1 не дошёл до середины и i2 не дошёл до конца
                if (comparator_func(strings_array[i1],strings_array[i2])<0)
                    temp[index++] = strings_array[i1++];  // заполняем участок результирующей последовательности
                else  temp[index++] = strings_array[i2++];
            }
            // Или i1 < m или i2 < r - только один из операторов while может выполниться
            while (i1 < m) temp[index++] = strings_array[i1++]; // заносим оставшиеся элементы сортируемых участков
            while (i2 < r) temp[index++] = strings_array[i2++]; // в результирующий массив
            l += step * 2; // перемещаемся на следующий сортируемый участок
            m += step * 2;
            r += step * 2;
        } while (l < array_size); // пока левая граница сортируемого участка - в пределах последовательности
...
void quick(strings_array_t strings_array, array_size_t array_size, comparator_func_t comparator_func) {
    ...
    int left = 0;     //Указатели в начало и в конец массива
    int right = array_size - 1;
    char *mid, *temp;

    mid = strings_array[array_size / 2];  //Центральный элемент массива
    //Делим массив
    do {    //Пробегаем элементы, ищем те, которые нужно перекинуть в другую часть
        ...
        if (left <= right) {     //Меняем элементы местами
    ...
    if(right > 0)     //"Левый кусок"
        quick(strings_array, right + 1, comparator_func);
    if (left < (int)array_size)       //"Правый кусок"
...
void radix(strings_array_t strings_array, array_size_t array_size, comparator_func_t comparator_func) {
    ...
    for (int i = (int) max_addit_arr - 1; i >= 0; i--) {
        ...
        if (comparator_func("a", "b") < 0)  // результат не нужен, служит для определения переданной функции
